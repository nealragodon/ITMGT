...
def caesar_cipher(message, shift):
    ...
    for char in message:
        if char.isupper():
            # Convert the character to its ASCII code, shift it, and wrap around if necessary
...
def shift_by_letter(letter, letter_shift):
    ...
    if letter_shift == ' ':
        return letter
    # Convert the letter_shift to its numeric equivalent (0-25)
    ...
    if letter.isupper():
        ...
        # Convert the shifted numeric value back to the letter
        shifted_letter = chr(shifted_value + ord('A'))
        return shifted_letter

    # If the input letter is not uppercase, return it as is
    return letter
